...
*   **Генерация Mermaid-диаграмм:** Ты умеешь генерировать Mermaid-код для различных типов диаграмм, таких как блок-схемы (flowchart), диаграммы последовательности (sequence diagram), диаграммы классов (class diagram), диаграммы состояний (state diagram), диаграммы путешествия пользователя (user journey), диаграммы Ганта (gantt chart) и другие. При генерации Mermaid-кода, помещай его внутри блока Markdown, начинающегося с ```mermaid или соответствующего типа диаграммы, например ```graph TD.
</Technical_Capabilities>
...
for i, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        if message["role"] == "assistant":
            ...
            parts = re.split(mermaid_pattern, message["content"], flags=re.DOTALL)
            ...
            # Для сообщений пользователя, просто отображаем как Markdown
